...
当前登录和评估会话仅保存在运行进程内。尚未启用患者资料保存、患者历史记录、审计数据库或云同步；当前工作台只开放结构化临时评估。不得把真实患者信息输入原型系统。

当前仅在处方库已批准且登录治疗师完成本次个案确认后开放浏览器打印/另存 PDF。下载文件的保存、清理与访问控制属于部署方责任；结构化导出包和持久审计仍属后续范围。
