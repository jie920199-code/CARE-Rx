...
export function MobileAssessment({ prescriptions }: { prescriptions: Prescription[] }) {
  ...
  return <main><header><div className="brand">CR</div><div><strong>CARE-Rx</strong><span>移动查房评审</span></div><em>仅限虚构病例</em></header>
    ...
    {submitted && matches.map((rx) => <section className="result prescription" key={rx.prescriptionId}><p className="version">{rx.prescriptionId} · {rx.version}</p><h2>{rx.trainingName}</h2><dl><dt>本次分类</dt><dd>{moduleId} · {level} · {assist} · {goal}</dd><dt>适用等级</dt><dd>{rx.applicableFunctionalLevels.join("、")}</dd><dt>辅助范围</dt><dd>{rx.assistanceLevels.join("、")}</dd><dt>起始体位</dt><dd>{rx.startPosition}</dd><dt>剂量</dt><dd>{rx.dose.repetitions.value}；{rx.dose.sets.value}组；{rx.dose.duration.value}{rx.dose.duration.unit}；{rx.dose.frequency.value}；{rx.dose.intensity.value}</dd></dl><details><summary>查看步骤与终止标准</summary><h3>训练步骤</h3><ol>{rx.steps.map((step: string) => <li key={step}>{step}</li>)}</ol><h3>终止标准</h3><ul>{rx.terminationCriteria.map((item: any) => <li key={item.code}>{item.label}</li>)}</ul></details>{!confirmed ? <button className="primary" type="button" onClick={() => setConfirmed(true)}>治疗师确认本次处方</button> : <div className="confirmed"><strong>本次已确认</strong><button type="button" onClick={() => window.print()}>打印 / 另存 PDF</button></div>}</section>)}
