...
export function MobileAssessment({ prescriptions, proposals }: { prescriptions: Prescription[]; proposals: Prescription[] }) {
  ...
  const matches = useMemo(() => safetyStatus === "clear" ? prescriptions.filter((item) => item.status === "approved" && item.clinicalReview?.status === "approved" && item.applicableModules.includes(moduleId) && item.applicableFunctionalLevels.includes(level) && item.assistanceLevels.includes(assist) && (!item.goalModes || item.goalModes.includes(goal))).slice(0, 5) : [], [prescriptions, moduleId, level, assist, goal, safetyStatus]);
