...
import { getPublicReviewSafetyStatus, matchApprovedPublicReviewPrescriptions } from "../lib/public-review-matcher.mjs";
...
export function MobileAssessment({ prescriptions, proposals }: { prescriptions: Prescription[]; proposals: Prescription[] }) {
  ...
  const [safety, setSafety] = useState(["unknown", "unknown", "unknown"]); const [submitted, setSubmitted] = useState(false); const [confirmedIds, setConfirmedIds] = useState<string[]>([]);
  const [feedbackCopied, setFeedbackCopied] = useState(false);
  const safetyStatus = getPublicReviewSafetyStatus(safety, assist);
  const matches = useMemo(() => matchApprovedPublicReviewPrescriptions({ prescriptions, moduleId, functionalLevel: level, assistanceLevel: assist, goalMode: goal, safetyStatus }), [prescriptions, moduleId, level, assist, goal, safetyStatus]);
  const proposalMatches = useMemo(() => safetyStatus === "clear" ? proposals.filter((item) => {
    const parent = prescriptions.find((rx) => rx.prescriptionId === item.sourcePrescriptionId);
    return item.reviewStatus === "pending_clinical_review" && item.module === moduleId && item.applicableFunctionalLevels.includes(level) && item.goalModes.includes(goal) && parent?.assistanceLevels.includes(assist);
  }).slice(0, 5) : [], [proposals, prescriptions, moduleId, level, assist, goal, safetyStatus]);
  const selectedLevel = levels.find((item) => item.id === level)!;
  const selectedAssistance = assistance.find((item) => item.id === assist)!;
  const reset = () => { setSubmitted(false); setConfirmedIds([]); };
  const toggleSelection = (prescriptionId: string) => setConfirmedIds((current) => current.includes(prescriptionId) ? current.filter((id) => id !== prescriptionId) : current.length < 3 ? [...current, prescriptionId] : current);
  ...
  return <main><header><div className="brand">CR</div><div><strong>CARE-Rx</strong><span>移动查房评审</span></div><em>公开评审版</em></header>
    ...
    {submitted && safetyStatus === "not_suitable" && <section className="result danger"><h2>AX 阻断：目前不宜实施</h2><p>AX 不是可执行辅助等级。系统不显示处方候选；请由治疗师复核暂停原因、照护条件及是否需要转诊或重新评估。</p></section>}
    {submitted && safetyStatus === "incomplete" && <section className="result warning"><h2>信息不足</h2><p>补全所有安全项目并由治疗师复核前，不得继续。</p></section>}
    {submitted && safetyStatus === "clear" && matches.length === 0 && <section className="result warning"><h2>没有可自动匹配的已批准候选</h2><p>当前模块、功能等级、辅助等级与目标模式未同时命中已批准适用范围，或处方尚未完成目标模式审核。系统按失败关闭处理，不得越级或自行套用其他处方。</p></section>}
    {submitted && safetyStatus === "clear" && matches.length > 0 && <section className="panel selection-summary"><h2>治疗师评审选择</h2><p>系统最多显示5个已批准库候选；请根据虚构病例选择1～3个用于评审。选择不等于临床签发，不得交付护工执行。不同方案剂量不得直接相加，组合后仍需治疗师复核总时长、重复内容与恢复反应。</p><strong>已选择 {confirmedIds.length} / 3</strong>{confirmedIds.length > 0 && <button type="button" onClick={() => window.print()}>打印评审组合 / 另存 PDF</button>}</section>}
    {submitted && matches.map((rx: Prescription) => { const selected = confirmedIds.includes(rx.prescriptionId); return <section className={`result prescription ${selected ? "confirmed-prescription" : "unconfirmed-prescription"}`} key={rx.prescriptionId}><p className="version">已批准库候选 · 未签发 · {rx.prescriptionId} · {rx.version}</p>{rx.focusRole && <span className={`focus-badge ${rx.focusRole}`}>{rx.focusRole === "core" ? "核心方案" : rx.focusRole === "auxiliary" ? "辅助方案" : "替代方案"}</span>}<h2>{rx.trainingName}</h2>{rx.inheritance && <p className="source-note">临床条款继承：{rx.inheritance.sourcePrescriptionId} · {rx.inheritance.sourcePrescriptionVersion}</p>}<dl><dt>本次分类</dt><dd>{moduleId} · {selectedLevel.id}（{selectedLevel.name}）· {selectedAssistance.id}（{selectedAssistance.name}）· {goal}</dd><dt>适用等级</dt><dd>{rx.applicableFunctionalLevels.join("、")}</dd><dt>辅助范围</dt><dd>{rx.assistanceLevels.join("、")}</dd><dt>起始体位</dt><dd>{rx.startPosition}</dd><dt>剂量</dt><dd>{rx.dose.repetitions.value}；{rx.dose.sets.value}组；{rx.dose.duration.value}{rx.dose.duration.unit}；{rx.dose.frequency.value}；{rx.dose.intensity.value}</dd></dl><details><summary>查看步骤、替代方案与终止标准</summary><h3>训练步骤</h3><ol>{rx.steps.map((step: string) => <li key={step}>{step}</li>)}</ol><h3>替代训练</h3><ul>{rx.alternatives.map((item: string) => <li key={item}>{item}</li>)}</ul><h3>终止标准</h3><ul>{rx.terminationCriteria.map((item: any) => <li key={item.code}>{item.label}</li>)}</ul></details><button className={selected ? "secondary" : "primary"} type="button" disabled={!selected && confirmedIds.length >= 3} onClick={() => toggleSelection(rx.prescriptionId)}>{selected ? "移出评审组合" : confirmedIds.length >= 3 ? "最多选择3个" : "纳入本次评审组合"}</button></section>; })}
